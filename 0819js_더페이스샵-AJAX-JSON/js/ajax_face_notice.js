(function($){
    ...
    //페이지 번호 그룹단위 페이지 지정
    var groupPageBtnList = 5;
    var groupPageBtnCurrentPage = Math.ceil((endRecord/pages)/groupPageBtnList);
...
/////////////////////////////////////////////////////////////////////
        ...
        function allBindFn(){
            ...
            //그룹페이지 한 화면에 표시할 버튼의 개수
            //한그룹페이지 10개만 보이고 나머지는 안보인다.
            //다음그룹페이지 10개 11~20 표시(10번 페이지 넘어가면 노출)
            //다음그룹페이지 10개 21~29 표시(20번 페이지 넘어가면 노출)
            //groupPageBtnList = 5; //버튼의 개수를 5개만  보이는 페이지 그룹
            groupPageBtnCurrentPage = Math.ceil((endRecord/pages)/groupPageBtnList); //3페이지(10개 묶음 그룹페이지)
            groupPageBtnStartNum = (groupPageBtnCurrentPage-1)*groupPageBtnList;
            groupPageBtnEndNum = groupPageBtnStartNum+groupPageBtnList;

            //끝페이지번호
            if( groupPageBtnEndNum > Math.ceil(a.length/pages) ){ //전체 버튼의 개수 = 자리올림(200 / 70)
...
////////////////////////////////////////////////////////////////////////////
        function sBindFn(){ //쇼핑공지
            ...
            //끝페이지번호
            if( groupPageBtnEndNum > Math.ceil(s.length/pages) ){ //전체 버튼의 개수 = 자리올림(200 / 70)
        ...
        function bBindFn(){ //브랜드 소식
            ...
            //끝페이지번호
            if( groupPageBtnEndNum > Math.ceil(b.length/pages) ){ //전체 버튼의 개수 = 자리올림(200 / 70)
